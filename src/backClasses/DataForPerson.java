...
public class DataForPerson {
 ...
 public Person getPerson(int idNum){
	 ...
	 try {
		...
		ResultSet rSet=stm.executeQuery("select p.id,p.person_name,"
				+ "p.person_surname,p.person_birth_date,"
				+ "p.person_sex,p.person_email,ph.person_photo from persons p,person_photoes ph where p.id="+idNum+" and "
						+ "p.id=ph.person_id ;");
